...
public strictfp class Range implements Serializable {
    ...
    public boolean intersects(double lower, double upper) {
        if (this.lower == this.upper) {
            return false; // Zero-length ranges don't intersect
        }
        if (lower <= this.lower) {
     ...
     * @return A new range subsuming both input ranges (possibly
     * <code>null</code>).
     */
     ...
     * @return A range which spans over the input range, and has been expanded
     * to contain the input value.
     */
     ...
     * For example: expand(new Range(2, 6), 0.25, 0.5) returns a range from 1 to
     * 8.
     *
     ...
     * positively (to the right) by the delta value. Is equivalent to
     * shift(base, delta, false) (does not allow zero crossing).
     *
     ...
     * false, any bound which crosses the zero mark after shifting (either from
     * negative to positive, or positive to negative), will become zero.
     *
     * @param base             the base range.
     * @param delta            the shift amount.
     * @param allowZeroCrossing a flag that determines whether or not the bounds
     *                         of the range are allowed to cross zero after
     *                         adjustment.
     * @return A range representing the base range shifted right by delta.
     */
    public static Range shift(Range base, double delta,
    ...
    /**
     * Returns the given value clamped within the specified lower and upper
     * bounds.
     *
